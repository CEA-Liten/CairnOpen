#ifndef MANUALCONSTRAINT_H
#define MANUALCONSTRAINT_H

#include "globalModel.h"
#include "MIPModeler.h"
#include "BusSubModel.h"

class MODELS_DECLSPEC ManualConstraint : public BusSubModel
{
public:
//----------------------------------------------------------------------------------------------------
    ManualConstraint(CairnObject* aParent);
    ~ManualConstraint();
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------
    void declareModelParameters()
    {
        ...
        //double
        addParameter("PenaltyCost", &mPenaltyCost, 1.e12, false, &mMaxFlexIntegrateConstraint, "Huge penalty Cost in Currency/kg - added by MaxFlexIntegrateConstraint in case of constraint overhead", SFunctionUnit({ eFTypeDivision, { pCurrency() }, "kg" }));
        ...
        // int
        addParameter("PeriodIntegrateConstraint", &mPeriodIntegrateConstraint, 0, false, true, "If different from 0 MaxIntegrateConstraint is applied over the window of size PeriodIntegrateConstraint. If mMaxIntegrateSeparateConstraint is true  the periods are separated", "Nb");
