        ...
        if (ptrExp1D) {
            if (port->Direction() == GS::KPROD() && variable == "Flow")
            {
                if (firstPort) {
                    ...
                    computeTime(true, mHorizon, mNpdtPast, *ptrExp1D, optSol, mChargingTime.at(0), mDischargingTime.at(0));
                    computeTime(false, *mptrTimeshift, mNpdtPast, *ptrExp1D, optSol, mChargingTime.at(1), mDischargingTime.at(1));
            ...
            else if (port->Direction() == GS::KPROD()) 
            {
                if (firstPort) {
                    memChargingTime = mChargingTime.at(0);
                    memDischargingTime = mDischargingTime.at(0);
                    computeTime(true, mHorizon, mNpdtPast, *ptrExp1D, optSol, mDischargingTime.at(0));
                    computeTime(false, *mptrTimeshift, mNpdtPast, *ptrExp1D, optSol, mDischargingTime.at(1));
                    if (mChargingTime.at(0) > memChargingTime && mDischargingTime.at(0) > memDischargingTime) {
                        firstPort = false;
                    }
                }
            ...
            else if (port->Direction() == GS::KCONS()) 
            {
                if (firstPort) {
                    memChargingTime = mChargingTime.at(0);
                    memDischargingTime = mDischargingTime.at(0);
                    computeTime(true, mHorizon, mNpdtPast, *ptrExp1D, optSol, mChargingTime.at(0));
                    computeTime(false, *mptrTimeshift, mNpdtPast, *ptrExp1D, optSol, mChargingTime.at(1));
                    if (mChargingTime.at(0) > memChargingTime && mDischargingTime.at(0) > memDischargingTime) {
                        firstPort = false;
                    }
                }
    ...
    for (const auto& port : mListPort) {
        ...
        if (ptrExp1D) {
            if (port->Direction() == GS::KPROD())
            {
                ...
                computeProduction(true, mHorizon, mNpdtPast, *ptrExp1D, optSol, aPort, bPort, mChargedEnergyMap[portId].at(0), mDischargedEnergyMap[portId].at(0));
                computeProduction(false, *mptrTimeshift, mNpdtPast, *ptrExp1D, optSol, aPort, bPort, mChargedEnergyMap[portId].at(1), mDischargedEnergyMap[portId].at(1));
                computeLvlProduction(true, mHorizon, mNpdtPast, *ptrExp1D, optSol, aPort, bPort, mNLevChargedEnergyMap[portId].at(0), mNLevDischargedEnergyMap[portId].at(0));
                computeLvlProduction(false, *mptrTimeshift, mNpdtPast, *ptrExp1D, optSol, aPort, bPort, mNLevChargedEnergyMap[portId].at(1), mNLevDischargedEnergyMap[portId].at(1));
