...
int main()
{
	...
	TESTAPI2("Check that EcoInvestModel is in list of params", TestUtils::contains(vParams, "EcoInvestModel"))

	t_list vIOs = vELY_PEM.get_VarList();
	TESTAPI2("Check that isInstalled is in list of IOs", TestUtils::contains(vIOs, "isInstalled"))

	t_list vIndicators = vELY_PEM.get_IndicatorNames();
	TESTAPI2("Check that Annual consumption of ElectricalEnergy UsedPower is in list of indicators", 
	...
	TESTAPI2("Check that Total Project env impact of AP is in list of indicators",
	...
	TESTAPI2FALSE("Check that Total Project env impact of AP is NOT in list of indicators",
		TestUtils::contains(vTecEcoIndicators, "Total Project env impact of Acidification#Accumulated Exceedance")
	)
	...
	vIndicators = vELY_PEM.get_IndicatorNames();
	TESTAPI2("Check that Annual consumption of ElectricalEnergy MaxUsablePower is in list of indicators",
