...
int main()
{
	...
	TESTAPIBOOL("check list of labels 1", TestUtils::compare_lists(m_Problem.get_Labels(), {}))

	m_Problem.set_Labels({ "country", "year", "cost"});
	m_Problem.add_Label("site");	
	m_Problem.remove_Label("cost");
	TESTAPIBOOL("check list of labels 2", TestUtils::compare_lists(m_Problem.get_Labels(), { "country", "year", "site" }))
	...
	TESTAPIBOOL("Verify the value of country of ELY_PEM", TestUtils::compare_scalar(vELY_PEM->get_LabelValue("country"), "France", eString))
	TESTAPIBOOL("check ELY_PEM labels", TestUtils::compare_dict(vELY_PEM->get_LabelValues(), { {"country", "France"}, {"year", "2000"}, {"site", ""} }))
	...
	TESTAPIBOOL("Verify the value of year of Elec_Bus", TestUtils::compare_scalar(vElec_Bus->get_LabelValue("year"), "1995", eString))
	TESTAPIBOOL("check Elec_Bus labels", TestUtils::compare_dict(vElec_Bus->get_LabelValues(), { {"country", "UK"}, {"year", "1995"}, {"site", "London"} }))
	...
	TESTAPIBOOL("check list of labels 3", TestUtils::compare_lists(m_Problem.get_Labels(), { "country", "year", "site" }))

	vELY_PEM = m_Problem.get_Component("ELY_PEM");
	TESTAPIBOOL("check ELY_PEM labels 2", TestUtils::compare_dict(vELY_PEM->get_LabelValues(), { {"country", "France"}, {"year", "2000"}, {"site", ""} }))

	vElec_Bus = m_Problem.get_Bus("Elec_Bus");
	TESTAPIBOOL("check Elec_Bus labels 2", TestUtils::compare_dict(vElec_Bus->get_LabelValues(), { {"country", "UK"}, {"year", "1995"}, {"site", "London"} }))
