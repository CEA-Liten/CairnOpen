...
/* This test reads the study formation_persee.json and formation_persee_dataseries.csv.
   ...
   The reference is generated using the GUI using the original formation_persee.json

   After, as an additional test: it saves the study as AfterReAddingComponent.json, then re-reads the file 
   and performs another simulation  before comparing with the reference formation_persee_Results_Reference.csv again
*/
...
int main()
{
	...
	int liRet = 0;
	bool lbRet = false;
	string lsComponentName = "H2_Load";
	t_list llSettingsType = { "params", "timeseries", "options" }; // params timeseries options
	...
	TESTAPI("read_model from the file path : " + vFileName,
		m_Problem = m_Persee.read_Study(vFileName)
	)

	CairnAPI::MilpComponentAPI vDelComp = m_Problem.get_Component(lsComponentName);
	TESTAPI("remove_component : " + lsComponentName,
		m_Problem.remove(vDelComp)
	)

	t_list ListAfterRemove = m_Problem.get_Components();
	t_list::iterator vIter = find(ListAfterRemove.begin(), ListAfterRemove.end(), lsComponentName);
	lbFound = (vIter != ListAfterRemove.end());
