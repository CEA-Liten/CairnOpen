...
int main()
{
	...
	std::string vFileName = StudyRoot + (std::string)"/formation_cairn_saved.json";
	string const TimeseriesFileName = StudyRoot + (std::string)"/formation_cairn_dataseries.csv";
	string const ResultFileName = StudyRoot + "formation_cairn_saved_results_Results.csv";
	...
	TESTAPI("read study file from the data: " + ReferenceStudyFileName,
		m_Problem = m_Cairn.read_Study(ReferenceStudyFileName)
	)
	...
	TESTAPI("read study file generated by m_Problem.save_Study: " + vFileName,
		m_Problem2 = m_Cairn2.read_Study(vFileName)
	)
