...
def problem():
    app_home = path.dirname(path.realpath(__file__))
    simu_full =  path.join(app_home, './data/formation_persee.json')    
    persee_instance = CairnAPI(False)
    problem = persee_instance.read_study(simu_full)    
    yield problem
...
def analyse_solutions(solution):
    print(solution.get_status())
    print(solution.get_nb_solutions())
    print(solution.get_optimal_solution())
...
def test_sequence_1(problem):        
    ...
    app_home = path.dirname(path.realpath(__file__))
    resultPath =  path.join(app_home, 'results')
    save(problem, resultPath + "./formation_persee_new.json")
...
def test_sequence_2(problem):    
    ...
    app_home = path.dirname(path.realpath(__file__))
    resultPath =  path.join(app_home, 'results')
    save(problem, resultPath +  "./formation_persee_modif.json")
...
def test_sequence_run_compare(problem):
    app_home = path.dirname(path.realpath(__file__))
    dataPath =  path.join(app_home, 'data')    
    load_timeseries(problem,dataPath + "./formation_persee_dataseries.csv")
    run(problem, "")

if __name__ == '__main__':
    app_home = path.dirname(path.realpath(__file__))
    simu_full =  path.join(app_home, './data/formation_persee.json')    
    timeseries =  path.join(app_home, './data/formation_persee_dataseries.csv')    
    ...
    #modify_port(problem)
    #save(problem, "../results/pythonAPI/formation_persee_new.json")
    ...
    #initialize(problem)
    #save(problem, "../results/pythonAPI/formation_persee_new_2.json")
    solution = run(problem, "testDir")
    #print(solution)
    #analyse_solutions(solution)
    #df = get_plan_results(problem).transpose()
